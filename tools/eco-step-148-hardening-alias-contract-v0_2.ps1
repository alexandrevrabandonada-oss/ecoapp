...
function EnsureRuntimeNodeJs([string]$file){
  if(!(Test-Path $file)){ return @{ ok=$false; reason="missing"; changed=$false } }
  $raw = Get-Content $file -Raw
  $changed = $false

  # edge -> nodejs (duas variantes, pra não precisar de regex com aspas misturadas)
  $reEdgeDq = 'export\s+const\s+runtime\s*=\s*"edge"\s*;?'
  $reEdgeSq = 'export\s+const\s+runtime\s*=\s*''edge''\s*;?'

  if ($raw -match $reEdgeDq) {
    $raw = [regex]::Replace($raw, $reEdgeDq, 'export const runtime = "nodejs";')
    $changed = $true
  }
  if ($raw -match $reEdgeSq) {
    $raw = [regex]::Replace($raw, $reEdgeSq, 'export const runtime = "nodejs";')
    $changed = $true
  }
  ...
  if($changed){
    ...
    return @{ ok=$true; reason="patched"; changed=$true }
  }
  return @{ ok=$true; reason="already_ok"; changed=$false }
}
...
function RunCmd([string]$label, [scriptblock]$sb){
  ...
  try {
    ...
    $verify += ("(erro: " + $_.Exception.Message + ")")
  }
}
