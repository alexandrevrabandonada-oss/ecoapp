...
$matches = [regex]::Matches($raw, "\$r\.Value\s*\+=").Count
...
$hadAdd = ($raw -match "function\s+AddReportLine\(")
...
try{
  ...
  if(Get-Command BackupFile -ErrorAction SilentlyContinue){
    $pCount = (Get-Command BackupFile).Parameters.Count
    $lst = [System.Collections.Generic.List[string]]::new()
    $fake = Join-Path $Root "__no_such_file__217.txt"
    try{
      if($pCount -eq 2){ BackupFile $fake $lst | Out-Null }
      elseif($pCount -eq 3){ BackupFile $fake (Join-Path $Root "tools\_patch_backup") $lst | Out-Null }
      elseif($pCount -ge 4){ BackupFile $fake (Join-Path $Root "tools\_patch_backup") "x" $lst | Out-Null }
