...
$rep = NewReport "eco-step-21b-pickup-requests-attach-receipt-and-pedidos-token-safe"
$log = @()
$log += "# ECO — STEP 21b — Fix parse PS + attach receipt + token no /pedidos"
$log += ""
...
# localizar arquivo com fetch(/api/pickup-requests) fora da API
$pedidosFile = $null
$roots = @()
...
$log += ("ui file (fetch): {0}" -f ($pedidosFile ? $pedidosFile : "(não achei; patch só API)"))
$log += ""
...
# build pick fields WITHOUT double-quote interpolation traps
$codeLine = ""
if($codeField){ $codeLine = (', ' + $codeField + ': true') }
$pubLine = ""
if($publicField){ $pubLine = (', ' + $publicField + ': true') }

if($txt -notmatch "ECO_STEP21B_ATTACH_RECEIPT_BEGIN"){
  $helper = @"
`n// ECO_STEP21B_ATTACH_RECEIPT_BEGIN
...
// ECO_STEP21B_ATTACH_RECEIPT_END
"@
...
if($idxGet -ge 0){
  ...
  if($idxBrace -ge 0){
    ...
    if($idxBraceClose -ge 0){

      # wrap first .findMany(...) in GET: ecoWithReceipt(ARG)
      ...
      # sanitize return NextResponse.json({ items });
      $getBlock = $txt.Substring($idxBrace, ($idxBraceClose - $idxBrace + 1))
      $pat = 'return\s+NextResponse\.json\(\s*\{\s*items\s*\}\s*\)\s*;'
      if([regex]::IsMatch($getBlock, $pat)){
        $replacement = @"
...
"@
        $getBlock2 = [regex]::Replace($getBlock, $pat, $replacement, 1)
        $txt = $txt.Substring(0, $idxBrace) + $getBlock2 + $txt.Substring($idxBraceClose+1)
...
# ========= PATCH UI fetch =========
$log += ""
$log += "## PATCH UI (/api/pickup-requests fetch header)"
if($pedidosFile){
  $log += ("Backup ui: {0}" -f (BackupFile $pedidosFile))
  $p = Get-Content -LiteralPath $pedidosFile -Raw

  if($p -notmatch "ECO_STEP21B_PEDIDOS_TOKEN"){
    $pedHelper = @"
`n// ECO_STEP21B_PEDIDOS_TOKEN
...
"@
  ...
  # patch first fetch('/api/pickup-requests'...) to include headers if missing
...
$log += "4) /pedidos: com localStorage eco_operator_token setado, deve mandar header x-eco-token"
$log += "5) Aba anônima: recibo privado não deve vazar code/shareCode"
$log += ""

WriteUtf8NoBom $rep ($log -join "`n")

Write-Host ("✅ STEP 21b aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
