...
Write-Host ("== eco-step-136-wide-shell-mural-mapa-v0_1 == " + $ts) -ForegroundColor Cyan
...
function PatchFile([string]$file, [scriptblock]$mutator, [string]$backupDir, [ref]$report) {
  if (!(Test-Path -LiteralPath $file)) {
    $report.Value += "[SKIP] missing -> " + $file
    return
  }
  $raw = Get-Content -Raw -LiteralPath $file
  if ($null -eq $raw) {
    $report.Value += "[SKIP] empty -> " + $file
    return
  }
  $next = & $mutator $raw
  if ($next -ne $raw) {
...
# paths
$backupDir = Join-Path $root ("tools\_patch_backup\eco-step-136-wide-shell-mural-mapa-v0_1-" + $ts)
$reportDir = Join-Path $root "reports"
EnsureDir $reportDir
$reportPath = Join-Path $reportDir ("eco-step-136-wide-shell-mural-mapa-v0_1-" + $ts + ".md")
$report = @()
...
# (1) create/update EcoWideStyles.tsx
EnsureDir $stylesDir
$stylesLines = @(
  ...
  "    '/* se algum wrapper estiver “matando” a largura via max-width */',",
  "    'main[data-eco-wide=\"1\"] * { box-sizing: border-box; }',",
  "    '/* OpenStreetMap embeds: garante largura cheia */',",
...
$stylesContent = ($stylesLines -join "`n")
...
# write report
$out = @()
$out += ("# eco-step-136-wide-shell-mural-mapa-v0_1 (" + $ts + ")")
$out += ""
$out += "## Mudanças"
$out += ""
$out += $report
...
Write-Host "  abrir /eco/mural (deve ficar bem mais largo no desktop)"
Write-Host "  abrir /eco/mapa (idem)"