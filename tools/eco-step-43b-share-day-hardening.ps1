...
$rep = NewReport "eco-step-43b-share-day-hardening"
$log = @()
$log += "# ECO — STEP 43b — Share Day Hardening"
$log += ""
...
try {
  # -------------------------
  # PATCH 1) eliminar bug: /s/dia/ virando regex
  # -------------------------
  $badLine = 'const ecoDayPublicSharePath = () => /s/dia/;'
  # IMPORTANT: aspas simples para NÃO interpolar ${...}
  ...
  $routeLines = @(
    ...
    '      (',
    '        <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: bg, color: off, padding: 64, boxSizing: "border-box", fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Helvetica Neue, Arial", border: `10px solid ${yellow}` }}>',
    '          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>',
    '            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>',
    '              <div style={{ fontSize: 22, letterSpacing: 2, color: gray }}>#ECO — Escutar • Cuidar • Organizar</div>',
    '              <div style={{ fontSize: 64, fontWeight: 900, lineHeight: 1.0 }}>FECHAMENTO<span style={{ color: yellow }}> DO DIA</span></div>',
    '              <div style={{ fontSize: 42, fontWeight: 800, color: off }}>{day}</div>',
    '            </div>',
    '            <div style={{ width: 120, height: 120, borderRadius: 999, border: `8px solid ${yellow}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 34, fontWeight: 900, color: yellow }}>{fmt.label}</div>',
    '          </div>',
    ...
    '        </div>',
    '      ),',
    '      { width: fmt.w, height: fmt.h, headers: { "cache-control": "public, max-age=0, s-maxage=3600, stale-while-revalidate=86400" } }',
    '    );',
    '  } catch (err: unknown) {',
  ...
  WriteUtf8NoBom $route ($routeLines -join "`n")
  $log += "- OK: route-day-card escrito."
  $log += ""
  ...
  WriteUtf8NoBom $smokePath ($smokeLines -join "`n")
  $log += "- OK: smoke atualizado."
  $log += ""

  WriteUtf8NoBom $rep ($log -join "`n")
  Write-Host ("✅ STEP 43b aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
