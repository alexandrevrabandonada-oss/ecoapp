...
$backupDir = Join-Path $Root ('tools/_patch_backup/' + $ts + '-eco-step-69-mapa-abandono-x-cuidado-v0_1')
$reportDir = Join-Path $Root 'reports'
EnsureDir $backupDir
EnsureDir $reportDir

Write-Host ('== eco-step-69-mapa-abandono-x-cuidado-v0_1 == ' + $ts)
...
# ------------------------------------------------------------
# API: /api/eco/points/map  (lista + filtros)
# ------------------------------------------------------------
$LApiMap = @(
...
'',
'  // bbox opcional: minLat,minLng,maxLat,maxLng',
...
'    }',
'',
'    // ordena: abertos primeiro, depois mais recentes',
...
# ------------------------------------------------------------
# API: /api/eco/points/confirm  (eu vi também)
# ------------------------------------------------------------
$LApiConfirm = @(
'import { NextResponse } from "next/server";',
'import { prisma } from "@/lib/prisma";',
...
'',
'  try {',
'    // tentativa: campos comuns de contagem',
...
'    }',
'',
'    // tentativa: meta Json (se existir)',
'    try {',
...
# ------------------------------------------------------------
# API: /api/eco/points/report  (criar ponto + dedupe raio/tempo)
# ------------------------------------------------------------
$LApiReport = @(
'import { NextResponse } from "next/server";',
'import { prisma } from "@/lib/prisma";',
...
'',
'  // bounding box aproximada (SQLite sem geo)',
...
'',
'    if (best) {',
'      // auto-conta como confirmação (best-effort)',
'      try {',
'        await fetch(new URL("/api/eco/points/confirm", req.url).toString(), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: String((best as any).id || "") }) });',
'      } catch {',
'        // ignore',
'      }',
...
'    }',
'',
'    // cria (campos mais comuns). Se algum não existir, Prisma pode falhar — então fazemos fallback progressivo.',
'    const baseData: any = {',
'      status: "OPEN",',
'      category,',
...
'    } catch {',
'      // fallback: tenta sem title/evidenceUrl',
'      try {',
...
'        Abandono = pontos abertos. Cuidado = resoluções e confirmação comunitária. Recibo é lei.',
'      </p>',
'      <MapaClient />',
'    </main>',
'  );',
...
'    setRepMsg("");',
'    try {',
'      if (!navigator.geolocation) throw new Error("Geolocation não disponível no navegador.");',
...
'          setRepLat(lat);',
'          setRepLng(lng);',
'          setRepMsg("Localização preenchida.");',
'        },',
'        (e) => {',
'          setRepMsg("Falha ao obter localização: " + String(e.message || e));',
'        },',
...
'      if (!Number.isFinite(lat) || !Number.isFinite(lng)) throw new Error("Preencha lat/lng (ou use ‘minha localização’).");',
...
'',
'      if (j.deduped) {',
'        setRepMsg("Já existe um ponto parecido aqui (dedupe). Contou como confirmação. Você pode abrir e clicar ‘Eu vi também’ de novo.");',
'      } else {',
...
'            <span>Dias</span>',
'            <input value={String(days)} onChange={(e) => setDays(Number(e.target.value) || 30)} style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Status</span>',
'            <select value={status} onChange={(e) => setStatus(e.target.value)} style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }}>',
...
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Bairro</span>',
'            <input value={bairro} onChange={(e) => setBairro(e.target.value)} placeholder="ex: Aterrado" style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Categoria</span>',
'            <input value={category} onChange={(e) => setCategory(e.target.value)} placeholder="ex: Entulho" style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
'        </div>',
'',
'        <div style={{ marginTop: 10, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>',
'          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="buscar (texto livre)" style={{ flex: "1 1 220px", padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
...
'',
'      <div style={{ padding: 12, border: "1px solid #e5e5e5", borderRadius: 12, background: "#fff" }}>',
'        <div style={{ fontWeight: 900, marginBottom: 8 }}>Registrar ponto (com dedupe anti-caos)</div>',
'        <div style={{ opacity: 0.8, fontSize: 12, marginBottom: 10 }}>',
'          Se já existir um ponto no mesmo lugar nas últimas 24h (raio 80m), o sistema evita duplicar e conta como confirmação.',
'        </div>',
'',
'        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Categoria</span>',
'            <select value={repCategory} onChange={(e) => setRepCategory(e.target.value)} style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }}>',
'              <option>Lixo acumulado</option>',
'              <option>Entulho/volumoso</option>',
'              <option>Queimada/fumaça</option>',
'              <option>Esgoto/vazamento</option>',
'              <option>Óleo/químico</option>',
'              <option>Sensível/industrial</option>',
'            </select>',
'          </label>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Bairro (opcional)</span>',
'            <input value={repBairro} onChange={(e) => setRepBairro(e.target.value)} placeholder="ex: Retiro" style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
'        </div>',
'',
'        <label style={{ display: "grid", gap: 6, marginTop: 10 }}>',
'          <span>Descrição (o que está acontecendo)</span>',
'          <textarea value={repText} onChange={(e) => setRepText(e.target.value)} rows={3} placeholder="Descreva o ponto e o risco (ex.: lixo + rato + crianças passando...)" style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'        </label>',
'',
'        <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 10, alignItems: "end" }}>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Lat</span>',
'            <input value={repLat} onChange={(e) => setRepLat(e.target.value)} placeholder="-22.5..." style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
'          <label style={{ display: "grid", gap: 6 }}>',
'            <span>Lng</span>',
'            <input value={repLng} onChange={(e) => setRepLng(e.target.value)} placeholder="-44.1..." style={{ padding: 10, borderRadius: 10, border: "1px solid #ccc" }} />',
'          </label>',
...
'          <div style={{ opacity: 0.7, fontSize: 12 }}>Dica: use “Abrir no mapa” para navegação e prova de local.</div>',
'        </div>',
'',
...
'                    Eu vi também ({Number(it.confirmations || 0)})',
'                  </button>',
...
# REPORT
# ------------------------------------------------------------
$rep = Join-Path $reportDir ('eco-step-69-mapa-abandono-x-cuidado-v0_1-' + $ts + '.md')
$repLines = @(
'# eco-step-69-mapa-abandono-x-cuidado-v0_1',
'',
'- Time: ' + $ts,
'- Backup: ' + $backupDir,
'- API: /api/eco/points/map',
'- API: /api/eco/points/report (dedupe raio 80m / 24h)',
'- API: /api/eco/points/confirm ("eu vi também")',
'- UI: /eco/mapa',
'',
'## Verify',
'1) Ctrl+C -> npm run dev',
'2) GET /api/eco/points/map?days=30&status=ALL -> ok:true',
'3) Abra /eco/mapa',
'4) Use "Minha localização" e registre um ponto',
'5) Registre de novo no mesmo lugar -> deve vir deduped:true e contar como confirmação (best-effort)',
''
)
...
Write-Host '[VERIFY] API: irm "http://localhost:3000/api/eco/points/map?days=30&status=ALL" -Headers @{Accept="application/json"}'