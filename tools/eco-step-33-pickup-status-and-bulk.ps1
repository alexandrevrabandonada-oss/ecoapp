...
function GetEnvTokenName(){
  ...
  # fallback: se existe no process env
  foreach($c in $cands){
...
function TryReplaceInFile([string]$file, [string]$from, [string]$to){
  $txt = Get-Content -LiteralPath $file -Raw
  if($txt -notmatch [regex]::Escape($from)){ return $false }
  $bk = BackupFile $file
  $txt2 = $txt.Replace($from, $to)
  WriteUtf8NoBom $file $txt2
  return $true
}

$rep = NewReport "eco-step-33-pickup-status-and-bulk"
$log = @()
$log += "# ECO — STEP 33 — Status/RouteDay + APIs triage/bulk"
$log += ""
...
# detectar Receipt fields (code/public) por heurística simples lendo schema
$schemaTxt = Get-Content -LiteralPath $schema -Raw
$receiptCodeField = "code"
if($schemaTxt -notmatch "(?m)^\s*model\s+Receipt\s*\{"){
  $receiptCodeField = "code"
} else {
...
if($receiptPublicField){ $receiptSelect = $receiptSelect + ", " + $receiptPublicField + ": true" }
...
function ecoIsOperator(req: Request): boolean {
  ...
  // dev-friendly: se não houver token configurado, permite (MVP local)
...
export async function PATCH(req: Request) {
  ...
  if (body && "routeDay" in body) data.routeDay = body.routeDay;

  // se marcou DONE, grava collectedAt; se não, não mexe (pra não apagar histórico sem querer)
  if (body?.status === "DONE") {
...
# --- PATCH operador triagem fetch (best-effort)
$log += ""
...
$log += "3) Teste /operador/triagem (deve continuar OK)"
$log += "4) Agora temos APIs:"
...
Write-Host "3) Teste /operador/triagem (deve abrir e listar)" -ForegroundColor Yellow
Write-Host "4) (API) PATCH bulk pronto p/ próximo tijolo de UI" -ForegroundColor Yellow