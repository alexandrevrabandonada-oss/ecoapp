...
if ($null -eq $pkgRaw) {
  ...
  try {
    ...
    if ($deps.Count -gt 0) { AddCode $r "txt" ($deps -join $nl) }
...
if ($null -eq $schemas -or $schemas.Count -eq 0) {
  ...
  if ($schemaRaw) {
    ...
    if ($models.Count -gt 0) { AddLine $r ("- models (schema[0]): " + $models.Count); AddCode $r "txt" (($models | Sort-Object) -join $nl) }
  }
}
...
else { AddLine $r ("- /eco pages: " + $ecoPages.Count); $rows = @(); foreach ($p in $ecoPages) { $rows += (RelPath $p.FullName) }; AddCode $r "txt" (($rows | Sort-Object) -join $nl) }
...
else { AddLine $r ("- /api/eco routes: " + $ecoApi.Count); $rows = @(); foreach ($p in $ecoApi) { $rows += (RelPath $p.FullName) }; AddCode $r "txt" (($rows | Sort-Object) -join $nl) }
...
Write-Host ""
Write-Host "[NEXT] Cole aqui no chat o conteudo do report (ou pelo menos: DATABASE_URL / DB files / Prisma / Routes)." -ForegroundColor Cyan
