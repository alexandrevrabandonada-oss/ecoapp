...
Write-Host ("== eco-step-99c-fix-support-relation-and-list2-strings-v0_2 == " + $ts)
...
function FixList2BrokenStrings([string]$raw2) {
  ...
  # pickModel(pc, " ecoPoint ", ...) com quebras/espacos
  $raw2 = [regex]::Replace(
    $raw2,
  ...
  # pc?.[" ecoPointConfirm "] com quebras/espacos
  $raw2 = [regex]::Replace(
    $raw2,
...
$backupDir = Join-Path $Root ("tools/_patch_backup/" + $ts + "-eco-step-99c-fix-support-relation-and-list2-strings-v0_2")
...
if (-not (Test-Path -LiteralPath $prisma)) {
  Write-Host "[WARN] prisma.cmd nao encontrado em node_modules/.bin"
} else {
...
# REPORT
$rep = Join-Path $reportDir ("eco-step-99c-fix-support-relation-and-list2-strings-v0_2-" + $ts + ".md")
$repText = @(
