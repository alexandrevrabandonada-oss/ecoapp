...
function AddH2([System.Collections.Generic.List[string]]$r, [string]$t) {
  AddLine $r ""
  AddLine $r ("## " + $t)
}
function AddCode([System.Collections.Generic.List[string]]$r, [string]$lang, [string]$text) {
  ...
  if ($null -ne $text -and $text.Length -gt 0) {
    $text.Split("`n") | ForEach-Object { AddLine $r $_ }
  }
  AddLine $r "```"
}
...
function ParseEnvLine([string]$raw, [string]$key) {
  ...
  foreach ($ln in $lines) {
    ...
    if ($t.StartsWith($key + "=")) {
      $v = $t.Substring(($key + "=").Length)
      # remove aspas simples/duplas nas pontas
      if ($v.Length -ge 2) {
...
function TryResolveSqlitePath([string]$url) {
  ...
  # relativo ao root (prisma usa relativo ao schema/WD; aqui a gente tenta root primeiro)
...
if ($null -eq $pkgRaw) {
  ...
  try {
    ...
    if ($null -ne $scripts) {
      $s = @()
      foreach ($k in ($scripts.PSObject.Properties.Name | Sort-Object)) {
        $s += ($k + " = " + [string]$scripts.$k)
      }
    ...
    if ($null -ne $pkg.dependencies) {
      foreach ($k in ($pkg.dependencies.PSObject.Properties.Name | Sort-Object)) {
        if ($k -in @("next","react","react-dom","@prisma/client","prisma","typescript")) {
          $deps += ($k + " = " + [string]$pkg.dependencies.$k)
        }
      }
    }
    if ($null -ne $pkg.devDependencies) {
      foreach ($k in ($pkg.devDependencies.PSObject.Properties.Name | Sort-Object)) {
        if ($k -in @("next","react","react-dom","@prisma/client","prisma","typescript")) {
          $deps += ($k + " = " + [string]$pkg.devDependencies.$k + " (dev)")
        }
      }
    }
    if ($deps.Count -gt 0) { AddCode $r "txt" ($deps -join "`n") }
  } catch {
    AddLine $r ("- erro parse package.json: " + $_.Exception.Message)
  }
}
...
$db = $db2
if ([string]::IsNullOrWhiteSpace($db)) { $db = $db1 }
...
if ($dbResolved) {
  AddLine $r ("- SQLite resolved path: `" + $dbResolved + "`")
}
...
if ($null -eq $schemas -or $schemas.Count -eq 0) {
  ...
  $list = @()
  foreach ($s in $schemas) { $list += ("  - " + (RelPath $s.FullName)) }
  AddCode $r "txt" ($list -join "`n")
  $schemaRaw = ReadRaw $schemas[0].FullName
  if ($schemaRaw) {
    # models
    $models = @()
    foreach ($ln in ($schemaRaw -split "`r?`n")) {
      $t = $ln.Trim()
    ...
    if ($models.Count -gt 0) {
      AddLine $r ("- models (schema[0]): **" + $models.Count + "**")
      AddCode $r "txt" (($models | Sort-Object) -join "`n")
    }
    # datasource url hints
    $urlHint = $null
    foreach ($ln in ($schemaRaw -split "`r?`n")) {
      $t = $ln.Trim()
      if ($t -match "^url\s*=") { $urlHint = $t; break }
    }
...
if ($null -eq $dbFiles -or $dbFiles.Count -eq 0) {
  ...
  foreach ($f in $dbFiles) {
    $rows += ((RelPath $f.FullName) + "  (" + [Math]::Round(($f.Length/1MB), 2) + " MB)")
  }
...
if ($null -eq $ecoPages -or $ecoPages.Count -eq 0) {
  AddLine $r "- src/app/eco: nao achei pages"
} else {
  AddLine $r ("- /eco pages: **" + $ecoPages.Count + "**")
  $rows = @()
  foreach ($p in $ecoPages) { $rows += (RelPath $p.FullName) }
...
if ($null -eq $ecoApi -or $ecoApi.Count -eq 0) {
  AddLine $r "- /api/eco routes: nao achei route.ts"
} else {
  AddLine $r ("- /api/eco routes: **" + $ecoApi.Count + "**")
  $rows = @()
  foreach ($p in $ecoApi) { $rows += (RelPath $p.FullName) }
  AddCode $r "txt" (($rows | Sort-Object) -join "`n")
}
...
if ($hintFiles.Count -gt 0) {
  $rows = @()
  foreach ($f in $hintFiles) { $rows += (RelPath $f.FullName) }
...
Write-Host ""
Write-Host "[NEXT] me mande esse arquivo de report aqui no chat (cola o conteúdo)." -ForegroundColor Cyan
Write-Host ("       (ele vai dizer onde está o dev.db e o formato canônico do Prisma)") -ForegroundColor Cyan

if ($OpenReport) {
  try { Start-Process -FilePath $reportPath | Out-Null } catch {}
}