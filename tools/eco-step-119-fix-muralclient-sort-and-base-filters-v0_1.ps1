...
# --- fallback mínimo (caso _bootstrap não tenha sido carregado por algum motivo)
...
$me = "eco-step-119-fix-muralclient-sort-and-base-filters-v0_1"
Write-Host ("== " + $me + " == " + $stamp)
...
function PatchFile([string]$filePath) {
  ...
  # 2) corrigir "p." dentro do comparator (quando isso aparece, derruba com p is not defined)
  # regra: trocar "p?.counts" por "a?.counts" (ou "b?.counts" dependendo do lado). Aqui fazemos o mínimo seguro:
  $raw = $raw.Replace("p?.counts?.", "a?.counts?.")

  # 3) garantir helpers dt/score (inserir após function num(...) se existir, senão inserir antes do primeiro export default / function principal)
  ...
  if (-not $hasScore) {
    $insert = @(
      ...
      "  // confirm/support/replicar (fallbacks se algum nome variar)",
    ...
    if ($idxNum -ge 0) {
      # acha o fim da função num (primeiro "}" depois do início)
      ...
      # fallback: inserir no topo
      $raw = $insert + $raw
    }
  }

  # 4) Base filters: se existir "const arr =" dentro do useMemo, trocar para "let arr =" e inserir filtros
  ...
  # 5) Normalizar comparator do sort (se achar campos antigos call/chamado) => usa score + createdAt
  if ($raw.Contains("counts?.call") -or $raw.Contains("chamadoCount") -or $raw.Contains("callCount")) {
    $raw = [Regex]::Replace(
      $raw,
...
"@,
  ...
  # 6) Se existe um .sort mas não usa score ainda, também normaliza (best-effort)
  if ($raw.Contains("arr.sort(") -and (-not $raw.Contains("score("))) {
    $raw = [Regex]::Replace(
      $raw,
      "arr\.sort\(\(a:\s*any,\s*b:\s*any\)\s*=>\s*\{[\s\S]*?\}\)\s*;",
      @"
...
$report += "- Normaliza sort por score (confirm+support+replicar) e desempate por createdAt."
$report += "- Adiciona filtros por base: confirmados (confirm>0) e acoes (score>0)."
$report += ""
$report += "## Verify"
$report += "1) Ctrl+C -> npm run dev"
$report += "2) abrir /eco/mural"
$report += "3) abrir /eco/mural/confirmados (deve mostrar só com confirm>0)"
$report += "4) abrir /eco/mural-acoes (se existir) (deve mostrar só com score>0)"
