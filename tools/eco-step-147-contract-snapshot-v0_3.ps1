...
if (Test-Path "src\app\eco") {
  $ecoPages = Get-ChildItem -Recurse -File "src\app\eco" | Where-Object { $_.Name -eq "page.tsx" } | Select-Object -ExpandProperty FullName
}
$ecoApi = @()
if (Test-Path "src\app\api\eco") {
  $ecoApi = Get-ChildItem -Recurse -File "src\app\api\eco" | Where-Object { $_.Name -match "^route\.tsx?$" } | Select-Object -ExpandProperty FullName
}

# ---- Get real IDs via Prisma (best-effort) usando JS temporário
...
'  await p.$disconnect();',
'})().catch(async (e)=>{ try{ await p.$disconnect(); }catch{} process.exit(0); });'
)
[IO.File]::WriteAllText($tmpJs, ($js -join "`n"), [Text.UTF8Encoding]::new($false))
...
function TryGetJson([string]$url) {
  try { return (Invoke-RestMethod -Uri $url -Headers @{ Accept="application/json" }) }
...
$snap = @{}
foreach ($t in $targets) { $snap[$t.name] = TryGetJson $t.url }
