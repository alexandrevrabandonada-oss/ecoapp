...
Write-Host ("== eco-step-106c-fix-seed-eco-pragma-string-v0_1 == " + $ts)
...
$backupDir = Join-Path $Root ("tools/_patch_backup/" + $ts + "-eco-step-106c-fix-seed-eco-pragma-string-v0_1")
...
# Write route.ts (avoid \"\" / \\\" issues; use PRAGMA with single quotes)
$tsLines = @(
...
'',
'  // VR (aprox) - so preenche se existir coluna',
...
$content = ($tsLines -join "`n") + "`n"
WriteUtf8NoBom $target $content
Write-Host ("[PATCH] rewrote " + $target)

$rep = Join-Path $reportDir ("eco-step-106c-fix-seed-eco-pragma-string-v0_1-" + $ts + ".md")
$repText = @(
"# eco-step-106c-fix-seed-eco-pragma-string-v0_1",
"",
"- Time: " + $ts,
"- Backup: " + $backupDir,
...
"",
"## Why",
"- Fix TS parse error caused by bad escaping in PRAGMA string (\\\"\\\" issue).",
"- Uses PRAGMA table_info('table') with single quotes to avoid parser confusion.",
"",
"## Verify",
