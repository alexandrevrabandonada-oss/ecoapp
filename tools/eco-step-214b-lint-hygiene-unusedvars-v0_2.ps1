...
$reportPath = Join-Path $Root ("reports\eco-step-214b-unusedvars-" + $stamp + ".md")
EnsureDir (Split-Path -Parent $reportPath)

# IMPORTANT: bootstrap expects $r as [ref] so it can use $r.Value
$r = [ref]@()

function Log([string]$s){ $r.Value += $s }

Log "# ECO STEP 214b — lint hygiene (unused vars -> `_`) — $stamp"
Log ""
Log "Root: $Root"
Log ""
...
function EnsureBackup([string]$p){
  if(-not $backedUp.ContainsKey($p)){
    # BackupFile (from _bootstrap) typically appends to $r.Value internally
    BackupFile $p
    $backedUp[$p] = $true
  }
}
...
function PatchLine([string]$relPath, [int]$lineNo, [scriptblock]$transform){
  ...
  if($lineNo -lt 1 -or $lineNo -gt $lines.Count){
    Log ("- [SKIP] out of range: " + $relPath + " (line " + $lineNo + " / " + $lines.Count + ")")
    return
  }
  ...
  if($new -ne $old){
    ...
    Log ("- [PATCH] " + $relPath + " :" + $lineNo)
    Log ("  - old: ``" + $old + "``")
    Log ("  - new: ``" + $new + "``")
  } else {
    Log ("- [OK]   " + $relPath + " :" + $lineNo + " (no change)")
  }
}
...
Log "## DIAG"
Log "- Goal: reduce ESLint noise by making unused vars/imports match /^_/"
Log ""
Log "## PATCH"
...
try {
  ...
  if(Test-Path $runner){
    Log "### eco-runner: -Tasks lint build"
    Log "~~~"
    pwsh -NoProfile -ExecutionPolicy Bypass -File $runner -Tasks lint build | ForEach-Object { Log $_ }
    Log "~~~"
    Log "exit: 0"
  } else {
    Log "- [SKIP] tools/eco-runner.ps1 not found"
  }
} catch {
  Log ("- [ERR] verify failed: " + $_.Exception.Message)
  Log ""
  throw
}
...
Log ""
Log "## NOTES"
Log "- This step only targets @typescript-eslint/no-unused-vars noise (underscore convention)."
Log "- Remaining warnings (react-hooks/exhaustive-deps, @next/next/no-img-element) are left for a later focused step."
Log ""

[IO.File]::WriteAllText($reportPath, ($r.Value -join "`n"), [Text.UTF8Encoding]::new($false))
