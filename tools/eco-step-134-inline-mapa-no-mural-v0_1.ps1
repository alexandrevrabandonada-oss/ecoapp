...
$me = "eco-step-134-inline-mapa-no-mural-v0_1"
...
if (Test-Path $bootstrap) {
  ...
  function BackupFile($path,$dir){ EnsureDir $dir; if(Test-Path $path){ Copy-Item -Force $path (Join-Path $dir ([IO.Path]::GetFileName($path))) } }
...
# create component (server component, simples e seguro)
$compLines = @(
  ...
  "          Dica: clique num ponto na lista do mapa pra focar o marcador.",
  "        </div>",
  "      </div>",
  "    </details>",
  "  );",
...
$comp = ($compLines -join "`n")
WriteUtf8NoBom $compFile $comp
Write-Host ("[PATCH] created -> " + $compFile)
...
for ($i=0; $i -lt $lines.Count; $i++) {
  if ($lines[$i] -match "MuralInlineMapa") { $hasImport = $true }
...
if (-not $hasImport) {
  ...
  if ($lastImport -ge 0) {
    $ins = $lastImport + 1
    $lines = $lines[0..$lastImport] + @("import MuralInlineMapa from ""./_components/MuralInlineMapa"";") + $lines[$ins..($lines.Count-1)]
    Write-Host "[PATCH] page.tsx import: OK"
  } else {
    $lines = @("import MuralInlineMapa from ""./_components/MuralInlineMapa"";") + $lines
    Write-Host "[PATCH] page.tsx import: OK (no topo)"
  }
}
...
$r += ""
$r += "## Mudanças"
$r += ("- created: " + (Resolve-Path $compFile).Path)
$r += ("- patched: " + (Resolve-Path $pageFile).Path)
$r += ""
$r += "## Verify"
