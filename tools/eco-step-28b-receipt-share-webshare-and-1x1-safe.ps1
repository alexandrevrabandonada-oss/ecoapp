...
function RemoveMarkedBlock([string]$txt, [string]$start, [string]$end){
  $pat = "(?s)\Q$start\E.*?\Q$end\E\s*"
  return [regex]::Replace($txt, $pat, "")
}

$rep = NewReport "eco-step-28b-receipt-share-webshare-and-1x1-safe"
$log = @()
$log += "# ECO — STEP 28b — Web Share (PNG) + Baixar 1:1 (ReceiptShareBar) — SAFE"
$log += ""
...
# detectar endpoint já usado (fallback /api/share/receipt-card)
$endpoint = "/api/share/receipt-card"
$m = [regex]::Match($txt, '\/api\/share\/receipt-card', 'IgnoreCase')
if($m.Success){ $endpoint = "/api/share/receipt-card" }
$log += ("Endpoint card: {0}" -f $endpoint)
$log += ""

$log += "## PATCH"

# 0) limpar tentativas anteriores (idempotente)
$txt2 = $txt
...
# 1) inserir helpers uma vez, depois do onCard3x4 / onWhatsApp / 'use client'
...
# 2) inserir botões após "Baixar card 3:4"
if($txt -notmatch "ECO_STEP28_SHARE_BUTTONS_START"){
  $needle = "Baixar card 3:4</button>"
  $idxBtn = $txt.IndexOf($needle)
  if($idxBtn -ge 0){
    $insertPos = $idxBtn + $needle.Length
    $buttons = @"
...
"@
    ...
    $log += "- WARN: não achei 'Baixar card 3:4</button>' — não inseri botões (helpers ficaram)."
  }
} else {
...
$log += "   - Compartilhar 3:4 / 1:1 (no celular/PWA abre share sheet; no desktop baixa)"

WriteUtf8NoBom $rep ($log -join "`n")

Write-Host ("✅ STEP 28b aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
