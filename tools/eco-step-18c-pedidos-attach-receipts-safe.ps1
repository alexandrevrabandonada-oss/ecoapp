...
$rep = NewReport "eco-step-18c-pedidos-attach-receipts-safe"
$log = @()
$log += "# ECO — STEP 18c — /api/pickup-requests lista já traz receipt + receiptCode"
$log += ""
...
"@
  ...
  foreach($it in $patterns){
    if(-not $did -and [regex]::IsMatch($txt2, $it.p)){
      $rg = [regex]::new($it.p)
      $txt2 = $rg.Replace($txt2, $it.r, 1)
      $did = $true
    }
  }

  if(-not $did){
    throw "Não achei return NextResponse.json(...) com 'items' em /api/pickup-requests para anexar receipt."
  }
...
$log += "3) /pedidos -> Fechar -> emitir -> voltar e ver Ver recibo na linha"
$log += ""

WriteUtf8NoBom $rep ($log -join "`n")

Write-Host ("✅ STEP 18c aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
Write-Host "PRÓXIMOS PASSOS:" -ForegroundColor Yellow
Write-Host "1) Reinicie o dev (CTRL+C): npm run dev" -ForegroundColor Yellow
Write-Host "2) Rode o smoke: pwsh -NoProfile -ExecutionPolicy Bypass -File .\tools\eco-smoke.ps1" -ForegroundColor Yellow
Write-Host "3) /pedidos -> Fechar -> emitir -> voltar e ver 'Ver recibo' na linha" -ForegroundColor Yellow