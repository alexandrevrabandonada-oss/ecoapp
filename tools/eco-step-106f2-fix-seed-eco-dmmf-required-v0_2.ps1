...
function BackupFile([string]$src, [string]$backupDir) {
  ...
  if (Test-Path $src) {
    ...
    Copy-Item -Force $src (Join-Path $backupDir $name)
  }
}
...
function FindRepoRoot([string]$start) {
  ...
  while ($true) {
    ...
    if ($parent -eq $p) { return (Resolve-Path $start).Path }
    $p = $parent
  }
}

$stamp = Get-Date -Format "yyyyMMdd-HHmmss"
$me = "eco-step-106f2-fix-seed-eco-dmmf-required-v0_2"

$repo = FindRepoRoot $Root
Write-Host ("== " + $me + " == " + $stamp)
Write-Host ("[DIAG] Repo: " + $repo)

$backupDir = Join-Path $repo ("tools\_patch_backup\" + $stamp + "-" + $me)
EnsureDir $backupDir

$target = Join-Path $repo "src\app\api\dev\seed-eco\route.ts"
...
'// Dev-only seed for ECO points/confirm/support/replicar — fills required scalar/enum fields via Prisma.dmmf safely.',
'',
...
WriteUtf8NoBom $target $ts
Write-Host ("[PATCH] rewrote " + $target)

# REPORT
$reportPath = Join-Path $repo ("reports\" + $me + "-" + $stamp + ".md")
$report = @()
$report += "# $me"
$report += ""
$report += "- Time: $stamp"
$report += "- Backup: $backupDir"
$report += "- Patched: src/app/api/dev/seed-eco/route.ts"
$report += ""
$report += "## What/Why"
$report += "- Seed estava quebrando por campos obrigatórios sem valor (ex.: kind/actor) e/ou valores inválidos (actor: String)."
$report += "- Agora usa Prisma.dmmf para preencher qualquer scalar/enum required sem default com valores seguros, sem PRAGMA."
$report += ""
$report += "## Verify"
$report += "1) Ctrl+C -> npm run dev"
$report += "2) irm 'http://localhost:3000/api/dev/seed-eco?n=3&confirm=1&support=1&replicar=1' -SkipHttpErrorCheck | ConvertTo-Json -Depth 80"
$report += "3) irm 'http://localhost:3000/api/eco/points?limit=5' | ConvertTo-Json -Depth 100"
$report += "4) abrir /eco/mural e /eco/mural/confirmados"
...
Write-Host "  irm 'http://localhost:3000/api/dev/seed-eco?n=3&confirm=1&support=1&replicar=1' -SkipHttpErrorCheck | ConvertTo-Json -Depth 80"
