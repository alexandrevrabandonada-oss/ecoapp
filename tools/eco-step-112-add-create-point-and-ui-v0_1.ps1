// eco-step-112-add-create-point-and-ui-v0_1
param(
...
$me = "eco-step-112-add-create-point-and-ui-v0_1"
Write-Host ("== " + $me + " == " + $stamp)
...
if ($raw -match "export\s+async\s+function\s+POST\s*\(") {
  ...
  $postBlock += ""
  $postBlock += "// --- added by tools/eco-step-112-add-create-point-and-ui-v0_1.ps1"
  $postBlock += "// POST /api/eco/points : cria ponto crítico (dev/prod) com defaults e validação leve."
  ...
  $postBlock += ""
  $postBlock += "    // model pick: prefer ecoCriticalPoint"
  ...
  $postBlock += "    if (!pointKey) {"
  $postBlock += "      return NextResponse.json({ ok: false, error: 'point_model_not_found' }, { status: 500 });"
  $postBlock += "    }"
  $postBlock += ""
  $postBlock += "    // Tenta descobrir enum de kind/status via Prisma.dmmf (sem precisar de import estático)"
  ...
  $postBlock += "      if (allowed.length) {"
  $postBlock += "        if (!allowed.includes(kind)) kind = allowed[0];"
  $postBlock += "      }"
  $postBlock += "    }"
  $postBlock += ""
  ...
  $postBlock += "      if (allowed.length) {"
  $postBlock += "        if (!allowed.includes(status)) status = allowed[0];"
  $postBlock += "      }"
  $postBlock += "    }"
  $postBlock += ""
  ...
  $postBlock += ""
  $postBlock += "    // Se existirem scalars obrigatórios sem default, preenche com valores seguros (best-effort)"
  ...
  $postBlock += "        if (f.name === 'id') continue; // deixa o prisma/DB lidar, ou outro código já põe"
  ...
  $newRaw = $raw + ($postBlock -join "`n")
  WriteUtf8NoBom $pointsApi $newRaw
...
$ui += '    setMsg(null);'
$ui += '    if (!navigator.geolocation) {'
$ui += '      setMsg("Seu navegador não tem geolocalização.");'
$ui += '      return;'
$ui += '    }'
$ui += '    navigator.geolocation.getCurrentPosition('
$ui += '      (pos) => {'
$ui += '        setF((x) => ({ ...x, lat: String(pos.coords.latitude), lng: String(pos.coords.longitude) }));'
$ui += '      },'
...
$ui += '        lng: Number(f.lng),'
$ui += '        note: f.note,'
$ui += '        photoUrl: f.photoUrl,'
$ui += '        actor: "app",'
$ui += '      };'
$ui += ''
$ui += '      const r = await fetch("/api/eco/points", {'
$ui += '        method: "POST",'
$ui += '        headers: { "Content-Type": "application/json" },'
$ui += '        body: JSON.stringify(payload),'
$ui += '      });'
...
$ui += '      setOpen(false);'
$ui += '      // garante que aparece mesmo se o mural buscar no client'
...
$ui += '      <button'
$ui += '        onClick={() => { setOpen(true); reset(); }}'
$ui += '        style={{ padding: "10px 12px", borderRadius: 12, border: "1px solid #111", background: "#111", color: "#fff", fontWeight: 900, cursor: "pointer" }}'
$ui += '      >'
$ui += '        ＋ Registrar ponto'
$ui += '      </button>'
$ui += ''
...
$ui += '            </div>'
$ui += ''
$ui += '            <div style={{ marginTop: 10, fontSize: 12, opacity: 0.85 }}>'
$ui += '              Dica: o servidor valida lat/lng e aplica defaults seguros (kind/status) se você digitar algo inválido.'
$ui += '            </div>'
$ui += '          </div>'
$ui += '        </div>'
$ui += '      ) : null}'
...
if (-not ($mural -match "MuralNewPointClient")) {
  ...
  # JSX: insere logo após <MuralNavPillsClient />
...
$report += "3) irm 'http://localhost:3000/api/eco/points?limit=5' | ConvertTo-Json -Depth 40 (deve crescer)"
$report += "4) Teste direto:"
$report += "   irm 'http://localhost:3000/api/eco/points' -Method Post -ContentType 'application/json' -Body '{\"lat\":-22.521,\"lng\":-44.105,\"kind\":\"LIXO_ACUMULADO\",\"status\":\"OPEN\",\"note\":\"teste\"}' | ConvertTo-Json -Depth 40"
$report += ""
