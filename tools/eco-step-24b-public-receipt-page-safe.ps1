...
$rep = NewReport "eco-step-24b-public-receipt-page-safe"
$log = @()
$log += "# ECO — STEP 24b — Página pública do Recibo (/r/[code]) (safe)"
$log += ""
...
export default async function PublicReceiptPage({ params }: { params: Promise<Params> }) {
  ...
  const CODE_FIELD = '__RECEIPT_CODE_FIELD__';
  ...
  const PUBLIC_FIELD = '__RECEIPT_PUBLIC_FIELD__';
...
"@
  ...
  $pageTsx = $pageTsx.Replace("__RECEIPT_CODE_FIELD__", $receiptCodeField)
  $pageTsx = $pageTsx.Replace("__RECEIPT_PUBLIC_FIELD__", $receiptPublicField)
...
$log += "3) Teste: http://localhost:3000/r/<CODE> (só abre se public=true)"
$log += "4) Aba anônima também deve abrir (sem token)."
$log += ""

WriteUtf8NoBom $rep ($log -join "`n")

Write-Host ("✅ STEP 24b aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
