...
function FindPrismaCmd(){
  $candidates = @("node_modules\.bin\prisma.cmd","node_modules\.bin\prisma")
...
$rep = NewReport "eco-step-10h-ensure-prisma-cli-and-push"
$log = @()
$log += "# ECO — STEP 10h — Ensure Prisma CLI + db push (fix status column drift)"
$log += ""
...
# Resolve caminho do sqlite para backup
$dbPath = $null
if($dbUrl -match "^file:(.+)$"){
  $p = $Matches[1].Trim()
  $p = $p.TrimStart("/")
...
# 1) garantir prisma CLI instalado
...
# 2) prisma db push (com accept-data-loss)
$log += "## DB PUSH"
...
Write-Host ("✅ STEP 10h aplicado. Report -> {0}" -f $rep) -ForegroundColor Green
