...
$backupDir = Join-Path $Root ("tools/_patch_backup/" + $ts + "-eco-step-61-critical-points-mvp-v0_1")
$reportDir = Join-Path $Root "reports"
EnsureDir $backupDir
EnsureDir $reportDir

Write-Host ("== eco-step-61-critical-points-mvp-v0_1 == " + $ts)
...
# Add enum + models if missing (append)
...
'',
'  try {',
'    // Dedupe simples: busca pontos recentes e mede distância no JS (dataset pequeno).',
...
'',
'  try {',
'    // tenta criar confirm único (pointId, actor); se já existir, não incrementa de novo',
...
'',
'function uid() {',
'  // id local (não é login). evita double-confirm por pessoa no mesmo browser.',
...
# Prisma migrate/generate (best-effort)
...
if (Test-Path $prismaCmd) {
  ...
  $rM = Run ("& '" + $prismaCmd + "' migrate dev --name eco_critical_points") $Root
  Write-Host $rM.Out
...
# Report
$rep = Join-Path $reportDir ("eco-step-61-critical-points-mvp-v0_1-" + $ts + ".md")
$repLines = @(
"# eco-step-61-critical-points-mvp-v0_1",
"",
"- Time: " + $ts,
"- Backup: " + $backupDir,
"",
