...
function FixBrokenRegexLiteral([string]$raw){
  # normaliza / \n confirm \n /i  => /confirm/i  (idem replicar/support)
  $pattern = '\/\s*\r?\n\s*(confirm|replicar|support)\s*\r?\n\s*\/i'
  return [regex]::Replace($raw, $pattern, '/$1/i')
}
...
# 3) confirm/replicar/support: fix regex literal quebrada por newline
UpdateFile "src\app\api\eco\points\confirm\route.ts" {
  param($raw)
  return FixBrokenRegexLiteral $raw
...
# VERIFY
RunCmd "npm run build" { npm run build } 320 ([ref]$verify)
