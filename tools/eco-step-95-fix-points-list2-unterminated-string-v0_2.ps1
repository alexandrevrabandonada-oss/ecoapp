...
Write-Host ('== eco-step-95-fix-points-list2-unterminated-string-v0_2 == ' + $ts)
...
$backupDir = Join-Path $Root ('tools/_patch_backup/' + $ts + '-eco-step-95-fix-points-list2-unterminated-string-v0_2')
...
# Fix pattern: pickModel(pc, "<newline> ecoPoint <newline>", [ ... ])
$pattern  = 'pickModel\(\s*pc\s*,\s*"\s*[\r\n]+\s*([A-Za-z0-9_]+)\s*[\r\n]+\s*",\s*\['
$before   = $raw
$raw      = [regex]::Replace($raw, $pattern, 'pickModel(pc, "$1", [')
$changed1 = ($raw -ne $before)

# Extra safeguard: pickModel(pc, "<newline> NAME <newline>", ...)
...
$rep = Join-Path $reportDir ('eco-step-95-fix-points-list2-unterminated-string-v0_2-' + $ts + '.md')
$repText = @(
...
Write-Host '[VERIFY] http://localhost:3000/eco/mural/confirmados'