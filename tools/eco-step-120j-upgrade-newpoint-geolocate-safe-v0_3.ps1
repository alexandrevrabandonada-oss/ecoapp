...
# try load bootstrap (ok if exists)
[void](TryDotSourceBootstrap $Root)

# --- FALLBACKS (define if missing)
...
$me = "eco-step-120j-upgrade-newpoint-geolocate-safe-v0_3"
Write-Host ("== " + $me + " == " + $stamp)
...
$target = Join-Path $Root "src\app\eco\mural\_components\MuralNewPointClient.tsx"
...
$report += "- Patched: src/app/eco/mural/_components/MuralNewPointClient.tsx (GPS + router.refresh)"
$report += ""
$report += "## Verify"
$report += "1) Ctrl+C -> npm run dev"
$report += "2) abrir /eco/mural"
$report += "3) clicar 📍 (permitir localização) -> depois ✅ Registrar"
$report += "4) conferir se o ponto aparece no mural"
...
Write-Host "  clicar 📍 (permitir localização) e depois ✅ Registrar"