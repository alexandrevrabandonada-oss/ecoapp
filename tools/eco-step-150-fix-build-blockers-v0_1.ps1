...
# =========================
# BOOTSTRAP (sem depender do prompt)
# =========================
...
function BackupFile([string]$file, [string]$bakRoot){
  ...
  $dst = Join-Path $bakRoot ($stamp + "-" + $safe)
...
# 1) MuralAcoesClient: garantir "use client" na PRIMEIRA LINHA (mesmo se tiver comentarios antes)
...
# 3) confirm/replicar/support: corrigir regex literal quebrada por newline: / \n confirm \n /i  -> /confirm/i
...
RunCmd "npm run build" { npm run build } 260 ([ref]$verify)

# Lint "scopado" (nao trava seu fluxo). Se eslint.cmd existir, roda só ECO e trunca.
