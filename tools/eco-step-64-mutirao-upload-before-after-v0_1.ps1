...
# --- bootstrap (preferir tools/_bootstrap.ps1 se existir)
...
$backupDir = Join-Path $Root ('tools/_patch_backup/' + $ts + '-eco-step-64-mutirao-upload-before-after-v0_1')
$reportDir = Join-Path $Root 'reports'
EnsureDir $backupDir
EnsureDir $reportDir

Write-Host ('== eco-step-64-mutirao-upload-before-after-v0_1 == ' + $ts)
...
# --- API: /api/eco/upload (salva em public/eco-uploads/...)
$LUpload = @(
...
WriteAllLinesUtf8NoBom $apiUpload $LUpload
Write-Host '[PATCH] wrote src/app/api/eco/upload/route.ts'

# --- PATCH MutiraoDetailClient: file upload antes/depois
$LDetail = @(
'"use client";',
'',
...
WriteAllLinesUtf8NoBom $detailClient $LDetail
Write-Host '[PATCH] wrote src/app/eco/mutiroes/[id]/MutiraoDetailClient.tsx'

# --- PATCH card: renderiza imagens quando existirem (prefixa origin se URL for relativa)
$LCard = @(
...
'            {beforeSrc ? (',
'              <img src={beforeSrc} alt="antes" style={{ width: "100%", height: "100%", objectFit: "cover" }} />',
'            ) : (',
...
'          <div style={{ width: boxW, height: boxH, borderRadius: 18, border: "2px solid #333", background: "#111", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>',
'            {afterSrc ? (',
'              <img src={afterSrc} alt="depois" style={{ width: "100%", height: "100%", objectFit: "cover" }} />',
'            ) : (',
'              <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "center" }}>',
...
WriteAllLinesUtf8NoBom $cardRoute $LCard
Write-Host '[PATCH] wrote src/app/api/eco/mutirao/card/route.tsx'

# --- Report
$rep = Join-Path $reportDir ('eco-step-64-mutirao-upload-before-after-v0_1-' + $ts + '.md')
$repLines = @(
'# eco-step-64-mutirao-upload-before-after-v0_1',
'',
'- Time: ' + $ts,
'- Backup: ' + $backupDir,
'',
'## Added',
'- API: POST /api/eco/upload (multipart/form-data -> public/eco-uploads/...)',
'',
'## Patched',
'- UI: /eco/mutiroes/[id] (upload ANTES/DEPOIS + preview)',
'- OG: /api/eco/mutirao/card (renderiza imagens quando existir)',
'',
'## Verify',
'1) Restart dev server',
'2) Abra /eco/mutiroes -> Abra um mutirão',
'3) Suba uma foto ANTES e uma DEPOIS',
'4) Clique "Salvar rascunho" (ou "Finalizar")',
'5) Abra a URL retornada (/eco-uploads/...) no navegador',
'6) Abra /api/eco/mutirao/card?format=3x4&id=... e confirme que as imagens aparecem',
''
)
...
Write-Host '[VERIFY] /eco/mutiroes/[id] -> upload antes/depois -> salvar -> card 3:4'