...
Write-Host ("== eco-step-100-fix-list2-confirm-groupby-field-v0_3 == " + $ts)
...
$backupDir = Join-Path $Root ("tools/_patch_backup/" + $ts + "-eco-step-100-fix-list2-confirm-groupby-field-v0_3")
...
# 1) Fix broken multiline string patterns:
#    pc?.["\n ecoPointConfirm \n"]  -> pc?.["ecoPointConfirm"]
...
# 2) Patch confirm counts block: try multiple field names for groupBy.by
$lines = $raw -split "`n"
$idxConfirmComment = -1
...
if ($start -lt 0 -or $end -lt 0) {
  ...
  if ($idxGB -ge 0) {
    ...
    for ($j = $idxGB; $j -lt [Math]::Min($lines.Count, $idxGB+90); $j++) {
      if ($lines[$j] -match 'return\s+map\s*;') { $end = $j; break }
    }
  }
}
...
for ($i=0; $i -lt $lines.Count; $i++) {
  if ($i -eq $start) {
    foreach ($x in $ins) { $new.Add([string]$x) }
    $i = $end
    continue
  }
  $new.Add([string]$lines[$i])
}

$raw2 = ($new -join "`n")
WriteUtf8NoBom $target $raw2

$rep = Join-Path $reportDir ("eco-step-100-fix-list2-confirm-groupby-field-v0_3-" + $ts + ".md")
$repText = @(
  '# eco-step-100-fix-list2-confirm-groupby-field-v0_3',
  '',
  ('- Time: ' + $ts),
  ('- Backup: ' + $backupDir),
  '',
  '## What was happening',
  '- Prisma error: groupBy(by: [criticalPointId]) falha quando o campo real tem outro nome (ex.: pointId).',
  '- Em alguns casos o arquivo ficou com strings quebradas em multiplas linhas (Unterminated string constant).',
  '',
  '## Changes',
  '- list2 route: normaliza strings quebradas para pc?.["ecoPointConfirm"] e pickModel(pc, "ecoPoint", ["ecoCriticalPoint","criticalPoint","ecoPoint","point"]) quando aparecer.',
  '- list2 route: counts.confirm agora tenta pointId / criticalPointId / ecoCriticalPointId / ecoPointId e usa o primeiro que funcionar.',
  '',
  '## Verify',
  '1) Ctrl+C -> npm run dev',
  '2) http://localhost:3000/api/eco/points/list2?limit=10 (sem erro do Prisma)',
  '3) http://localhost:3000/eco/mural/confirmados (sem 500)',
  '',
  '## Notes',
  '- Este patch agrupa no dataset todo (sem where in ids) para evitar depender do nome da variavel de ids no arquivo.'
) -join "`n"
WriteUtf8NoBom $rep $repText

Write-Host ("[OK] Patched: " + $target)
Write-Host ("[DIAG] fixedStrings=" + $fixedStrings)
