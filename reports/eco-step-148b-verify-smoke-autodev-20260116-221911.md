# eco-step-148b — verify + smoke (auto dev) — 20260116-221911
...
./src/app/eco/mutiroes/[id]/MutiraoDetailClient.tsx:67:36
Type error: Cannot find name '__DEP__'.

  65 |   // eslint-disable-next-line react-hooks/set-state-in-effect
  66 |   // eslint-disable-next-line react-hooks/set-state-in-effect
> 67 |   useEffect(() => { refresh(); }, [__DEP__]);
     |                                    ^
  68 |   function toggle(k: string) { setCheck((prev: AnyObj) => ({ ...prev, [k]: !prev?.[k] })); }
  69 |
  70 |   async function saveDraft() {
